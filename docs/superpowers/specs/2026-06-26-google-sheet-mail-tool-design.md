...
- If recovery email is empty, generate one from the email username plus a random number and one of these domains: `fivermail.com`, `tupmail.com`, `givmai.com`, `gimpmail.com`.
- Use one domain per 500 source rows, then move to the next domain and loop after the fourth domain.
